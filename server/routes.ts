...
import { insertUserSchema, insertCandidateSchema, insertVoteSchema } from "@shared/schema";
import { z } from "zod";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/candidates", async (req: Request, res: Response) => {
    try {
      ...
      if (gradeParam && !isNaN(Number(gradeParam))) {
        ...
        candidates = await storage.getCandidatesByGrade(grade);
      } else {
  ...
  app.delete("/api/admin/candidates/:id", async (req: Request, res: Response) => {
    try {
      const id = Number(req.params.id);
